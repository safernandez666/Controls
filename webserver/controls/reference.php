  	...
  	<?php echo "<input type='hidden' id='nik' name='nik' value='".strip_tags($_GET["nik"],ENT_QUOTES)."'/>" ?>
			...
			<table class="table table-striped table-hover">
				...
				$nik = mysqli_real_escape_string($con,(strip_tags($_GET["nik"],ENT_QUOTES)));
              	...
              	if($filter){
				$sql = mysqli_query($con, "SELECT *  FROM referencias  WHERE mes < '13' AND id_control= '$nik' AND status='$filter' ORDER BY mes ASC");
               	}else{
					$sql = mysqli_query($con, "SELECT * FROM referencias WHERE mes < '13' AND id_control='$nik' ORDER BY mes ASC");
				}
